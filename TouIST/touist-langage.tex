...
~~~~~~~~~~~~~~~~\textbar{}~EOF%mdk
\end{mdpre}\noindent Un fichier {\scshape TouIST} est une liste séparée par des blancs\mdfootnote{2}{\noindent Un blanc est soit un espace, une tabulation ou une%mdk
\label{fn-whitespace}%mdk%mdk
}
d'assignements et de formules. Les assignements sont globaux et peuvent
être mélangés avec les formules tant qu'ils ne sont pas imbriqués dans des
formules (pour les variables locales, il est possible d'utiliser \mdcode{{\mdcolor{navy}let}};
voir Sec.~\mdref{let-binding}{2.2.3}).

Les commentaires commencent par "\mdcode{{\mdcolor{darkgreen};;}}". Deux anti-slashs (\mdcode{\textbackslash{}\textbackslash{}}) dans
une formule produira une nouvelle ligne dans la vue $\mbox{\LaTeX}$.%mdk

nouvelle ligne.%mdk

\noindent\textbf{Note}.
La liste de formules séparée par des blancs sera au final transformée en
conjonction de formules ; cela évite les nombreux \mdcode{and} entre les
formules de lignes différentes.%mdk

\textbf{Attention :} chaque formule de cette liste est implicitement
parenthésée :%mdk
\begin{mdpre}%mdk
\noindent a~or~b\\
c~=\textgreater{}~a%mdk
\end{mdpre}\noindent sera transformé en :
\begin{mdpre}%mdk
\noindent(a~or~b)~and~(c~=\textgreater{}~a)%mdk
\end{mdpre}\label{newline-and-note}%mdk%mdk

\section{2.2.\hspace*{0.5em}Les variables}\label{sec-les-variables}%mdk%mdk

\noindent Une variable, par exemple \mdcode{{\mdcolor{purple}\$var}}, permet entre autres d'utiliser les
opérateurs \mdcode{{\mdcolor{navy}bigand}} et \mdcode{{\mdcolor{navy}bigor}}. Plus récemment, nous avons introduit
la possibilité d'y assigner une formule (formules entre guillemets, voir
Sec.~\mdref{quoted-formulas}{2.7.7}).%mdk

Nous décrirons d'abord toutes les formes que peut prendre une variable.
Ensuite, nous détaillerons comment assigner une variable à une valeur
(par des assignations globales ou locales).%mdk

\subsection{2.2.1.\hspace*{0.5em}Syntaxes des variables}\label{sec-syntaxes-des-variables}%mdk%mdk
\begin{mdpre}%mdk
...
~~~~\textbar{}~"""{\mdcolor{purple}\textless{}formula-simple\textgreater{}}"""~{\mdcolor{darkgreen}\textless{}-~formule~entre~guillemets~(TouIST~\textgreater{}=~3.5.1)}\\
...
\mddefterm{\noindent{\bfseries Les variables simples (\textquotedblleft{}simple-var\textquotedblright{})}}%mdk

\begin{mdbmarginx}{}{}{}{1.5em}%mdk
\begin{mddefdata}%mdk
Une variable simple est de la forme \mdcode{{\mdcolor{purple}\$my\_var}}. Dans une formule, une%mdk
\end{mddefdata}%mdk
\end{mdbmarginx}%mdk
%mdk
\end{mddefinitions}%mdk

\noindent{}     variable simple devrait toujours être une proposition ou une
    ~\mdref{quoted-formulas}{formule entre guillemets}. Dans une expression, une
     variable simple peut contenir un entier, une virgule flottante, une
     proposition, un booléen ou un ensemble.
* Variable n-uplet (peut être vu comme un \emph{prédicat})
  : Une variable n-uplet est une variable simple suivie d'une liste de d'indices
    séparés par des virgules et entre parenthèses, par exemple,
    \mdcode{~{\mdcolor{purple}\$var}({\mdcolor{purple}\$i},a,{\mdcolor{purple}4})}. La variable principale (\mdcode{{\mdcolor{purple}\$var}})
    doit toujours contenir une proposition. Les indices imbriqués (par exemple \mdcode{{\mdcolor{purple}\$i}})
    peuvent être des entiers, des flottants, des propositions ou des booléens. 
  : Une variable de tuple sera toujours transformée en une proposition ou à une
    \mdref{quoted-formulas}{formule entre guillemets}. Par exemple, si
    \mdcode{{\mdcolor{purple}\$var}=p}et \mdcode{{\mdcolor{purple}\$i}=q}, il sera transformé en \mdcode{p(q,a,{\mdcolor{purple}4})}
  : Les variables n-uplets ne sont pas (encore) compatibles avec la notation
    set-builder (cf. Sec.~\mdref{set-builder}{2.6.3}). Si l'un des indices est un ensemble,
    l'ensemble restera tel quel.%mdk

Quelques exemples de variables:%mdk
...
\subsection{Assignements globaux}\label{global-var}%mdk%mdk

\noindent Nous appelons \guillemotleft{} variables globales \guillemotright{}%mdk

{\color{red}
\noindent
e call \textquotedblleft{}global variables\textquotedblright{} any variable that is assigned at the same
...
\noindent
hesubformulas \mdcode{a~and~(b~or~c)}, \mdcode{d}, \mdcode{e~or~f}, \mdcode{h~\textless{}=\textgreater{}~i},
