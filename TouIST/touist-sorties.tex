...
QDIMACS) comme langage d'entrée. Il suffit de donner la sortie DIMACS de \touist directement à un solveur et d'utiliser la table de correspondance (voir Section~\mdref{mapping-table}{3.2.2.1}).
...
solver externally as it is included with \mdcode{touist} (see Section~\mdref{usage-qbf-solver}{3.2.5.1}).%mdk
\begin{mdpre}%mdk
...
Boolean Formulas (QBF) grammars can also be used (see Sections~\mdref{usage-smt}{3.2.4} and
\mdref{usage-qbf}{3.2.5}).%mdk%mdk
...
\mdref{sec-propositional-logic-formulas}{2.7.4}. This mode is enabled by
...
\noindent The language accepted by the SMT mode is described in~\mdref{sec-smt-formulas}{2.7.5}.%mdk
...
directly to the solver and use the mapping table (see Section~\mdref{mapping-table}{3.2.2.1}).
