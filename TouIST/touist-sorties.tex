...
\paragraph{Installation}\label{sec-installation}%mdk%mdk
...
\paragraph{Usage}\label{cli}%mdk%mdk
...
%sub
\paragraph{Exit status}\label{sec-exit-status}%mdk%mdk
...
%sub
\paragraph{Usage for propositional logic (SAT mode)}\label{usage-sat}%mdk%mdk
...
\textbf{\mdcode{--solve~--interactive}} allows the user to display the models one after
...
%sub
\paragraph{Other general options}\label{usage-general-options}%mdk%mdk
...
%sub
\paragraph{Usage for Satisfiability Modulo Theory (SMT mode)}\label{usage-smt}%mdk%mdk
...
%sub
\paragraph{Usage for quantified boolean logic (QBF mode)}\label{usage-qbf}%mdk%mdk
...
\paragraph{Using external solvers from \mdcode{touist} (\mdcode{--solver})}\label{sec-using-external-solvers-from-touist---solver}%mdk%mdk
...
\begin{mdpre}%mdk
\noindent brew~install~depqbf\\
touist~test/qbf/allumettes2.touist~--qbf~--solver="depqbf\\
\qquad\qquad --qdo~--no-dynamic-nenofex"%mdk
\end{mdpre}%mdk
...
%sub
\paragraph{One single syntax error per run}\label{sec-one-single-syntax-error-per-run}%mdk%mdk
