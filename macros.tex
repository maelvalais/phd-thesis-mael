...
\newcommand{\tagrule}[1]{\vspace{0.5em}\noindent(#1)~}
...
\newcommand{\stepcte}[1]{S_{#1}}
\newcommand{\steplevel}[2]{S_{#2}^{#1}}
\newcommand{\instep}[2]{(#1 \in \step{#2})}
\newcommand{\instepcte}[2]{(#1 \in \stepcte{#2})}
...
\newcommand {\openstepcte}[2] {\textit{open}_{#1,\stepcte{#2}}} % open (in step)
